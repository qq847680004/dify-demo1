...
**Goal:** 生成可导入 Dify 1.16 的 Workflow DSL：输入**一张询价图片**（及可选客户名回退开关），结构化抽出与「询价字段抽取」**完全相同**的 11 键 `result_json`（每键 `{ value, confidence }`）。
...
- `end` 只暴露 `result_json`（string），键集合与文本抽取完全一致
...
- 视觉模型：`read_image` **必须**选带 Vision 标签的模型。现有占位 `qwen3.7-plus` **不一定**具备视觉；实施时对照 skill / 插件已列模型名，只换 `model.name`，不编造 `provider_id`。`extract_fields` 可继续用与文本抽取相同的 `qwen3.7-plus`（只吃转写文本）
...
- 缺省：`value=""` 且 `confidence="置信低"`
- 封闭枚举、地点 en/zh、模糊客户名回退：全部照抄文本抽取的 `build_json`，不要分叉

视觉转写失败或图中无字：仍输出完整 11 键空值 JSON，工作流本身成功结束。
...
| `extract_fields` | `parameter-extractor` | `query: [read_image, text]`；instruction 引用 `{{#start.fuzzy_match_customer#}}` | 与文本抽取相同的业务字段 + `{field}_confidence` + `customer_person_name` / `customer_email` + `__is_success` |
...
- [x] **Step 3:** 确认视觉模型 `name`：优先工作区/插件已列的 tongyi 视觉模型；若无把握，在 YAML 用明确占位并在交付说明写「导入后改选带 Vision 标签的模型」。禁止编造 marketplace identity。
...
- [ ] **Step 3:** 交付说明：导入后重连模型凭据；`read_image` 必须换成/确认视觉模型；试跑一张真实询价截图。
...
| 功能与文本抽取相同 | 复制 extract + build_json；同一 `result_json` |
